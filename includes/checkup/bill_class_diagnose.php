...
class wpmemory_Bill_Class_Diagnose
{
    ...
    public function site_health_tab_content($tab)
    {
        ...
        if ("Critical Issues" !== $tab) {
            ...
            <strong>
            <?php 
                echo esc_attr__("Displaying the latest recurring errors from your error log file and eventually alert about low WordPress memory limit is a courtesy of plugin", 'wp-memory');
                echo ': '.esc_attr($this->global_plugin_slug).'. ';
                echo esc_attr__("Disabling our plugin does not stop the errors from occurring; 
                it simply means you will no longer be notified here that they are happening, but they can still harm your site.", 'wp-memory');
            ?>
            </strong>
            </p>
                            ...
                            for ($i = 0; $i < $total; $i++) {
                                ...
                                if ($is_javascript) {
                                    ...
                                    if (preg_match($pattern, $line, $matches)) {
                                        ...
                                        /*              
                                            0 => '[29-Sep-2023 11:44:22 UTC] PHP Parse error:  syntax error, unexpected \'preg_match\' (T_STRING) in /home/realesta/public_html/wp-content/plugins/wp-memory/functions/functions.php on line 2066',
                                            1 => 'Parse error',
                                            2 => ' syntax error, unexpected \'preg_match\' (T_STRING)',
                                            3 => 'home/realesta/public_html/wp-content/plugins/wp-memory/functions/functions.php',
                                            4 => '2066',
                                            */
    ...
    public function custom_help_tab()
    {
        ...
        if ("site-health" === $screen->id) {
            ...
            $message = esc_attr__("These are critical issues that can have a significant impact on your site's performance. They can cause many plugins and functionalities to malfunction and, in some cases, render your site completely inoperative, depending on their severity. Address them promptly.", 'wp-memory');
            $screen->add_help_tab([
                "id" => "custom-help-tab",
...
*/
$diagnose_instance = wpmemory_Bill_Class_Diagnose::get_instance(
    $notification_url,
    $notification_url2,
    $plugin_text_domain,
    $plugin_slug
